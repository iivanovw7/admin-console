...
import Warning from '../../../../components/UI/Dialogs/Warning';
...
const setup = () => {
  ...
  describe('Should render Warning popup with styles and functions', () => {
    ...
    it('Should render main Dialog', () => {
      ...
      expect(wrapper.find(Dialog)).not.toBe(null);
      expect(wrapper.find(Dialog)).not.toBe(undefined);
    ...
    it('Should render main Modal window', () => {
      wrapper = mount(<Composition {...props} />);
      expect(wrapper.find(Modal)).not.toBe(null);
      expect(wrapper.find(Modal)).not.toBe(undefined);
    ...
    it('Should render component with correct text', () => {
      ...
      expect(wrapper.find('button').first().text()).toBe('No');
      expect(wrapper.find('button').last().text()).toBe('Yes');
    });
